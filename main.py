    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if batch_mol.x.shape[0] == 1:
            ...
            with torch.set_grad_enabled(False):
                if args.use_prot:
                    pred = model(batch_mol, batch_prot) #Mirar como funciona model, por qué salen 2 por cada cosa (es porque la primera corresponde a probabilidad de ser 0 y la otra a 1 (0 es no interacción y 1 sí es interacción?))
                else:
                    pred = model(batch_mol)
...
def main():
    ...
    if args.resume:
        ...
        # Load pre-trained LM module
            ...
            model_path = (
                "BINARY_{}/Fold{}/model_ckpt/Checkpoint_valid_best.pth".format(
