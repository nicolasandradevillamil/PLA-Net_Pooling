...
def eval(model, device, loader, num_classes, args):
    ...
    # breakpoint()
    test_model_path = os.path.join("./log",
        args.save
    )
    ...
    prop_predictor1.load_state_dict(torch.load(test_model_path1)["model_state_dict"], strict=False)
    prop_predictor1.to(device)

    prop_predictor2.load_state_dict(torch.load(test_model_path2)["model_state_dict"], strict=False)
    prop_predictor2.to(device)

    prop_predictor3.load_state_dict(torch.load(test_model_path3)["model_state_dict"], strict=False)
    prop_predictor3.to(device)

    prop_predictor4.load_state_dict(torch.load(test_model_path4)["model_state_dict"], strict=False)
